...
function index(){
    ...
    list($rows, $totalItems) = Albums::pagelist($submits, $pageID, $pageRows);
...
function m_edit(){
    ...
    switch( $type ){
        case 'active':
            $errmsg = Albums::setActive($items);
            break;
        case 'inactive':
            $errmsg = Albums::setInactive($items);
            break;
    }
    if( $errmsg === true ){
