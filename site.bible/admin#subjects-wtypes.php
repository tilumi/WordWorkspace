...
APP::$pageTitle = '禮拜類型';
APP::$mainTitle = '禮拜類型 Worship Types';
APP::$mainName = '禮拜類型';
$menu_id = 4;
...
function index(){
    ...
    $form->addElement('text', 'key', '代號', array('class'=>'input-long'));
    ...
    foreach($submits as $key=>$value){
        if( $value==='' ){ continue; }
        if( $key=='key' ){ $searchInfo[]='<u>代號</u> 含 "<span>'.$value.'</span>" '; }
    ...
    list($rows, $totalItems) = SubjectsWtypes::pagelist($submits, $pageID, $pageRows);
...
function add(){
    ...
    $form->addElement('text', 'id', '代號', array('class'=>'input-short'));
    $form->addElement('text', 'name', '類型名稱', array('class'=>'input-medium'));
    ...
    $form->addRule('id', '代號 必填', 'required', null, 'client');
    $form->addRule('id', '代號 必須是英文或數字', 'alphanumeric', null, 'client');
    $form->addRule('id', '代號至多20個字', 'maxlength', 20, 'client');
    $form->addRule('name', '類型名稱 必填', 'required', null, 'client');
    $form->addRule('name', '類型名稱至多255個字', 'maxlength', 255, 'client');
    $form->registerRule('is_allowed_id', 'callback', '_is_allowed_id' );
    $form->addRule('id', '這個代號已被使用，請換一個', 'is_allowed_id', null);
    ...
    if( count($submits)>0 ){
        ...
        if( $form->validate() ){
            $errmsg = SubjectsWtypes::add($submits); 
            if( $errmsg === true ){
...
function edit(){
    ...
    $data = SubjectsWtypes::findById($id);
    ...
    $form->addElement('text', 'sort', '排序', array('class'=>'input-short'));
    $form->addElement('static', '', '代號', $data['id']);
    ...
    if( count($submits)>0 ){
        ...
        if( $form->validate() ){
            $errmsg = SubjectsWtypes::edit($submits); 
            if( $errmsg === true ){
...
function delete(){
    ...
    if( count($submits)>0 ){
        ...
        if( $form->validate() ){
            $errmsg = SubjectsWtypes::delete($submits); 
            if( $errmsg === true ){
...
function m_edit(){
    ...
    switch( $type ){
        case 'active':
            $errmsg = SubjectsWtypes::setActive($items);
            break;
        case 'inactive':
            $errmsg = SubjectsWtypes::setInactive($items);
            break;
    }
    if( $errmsg === true ){
...
function m_delete(){
    ...
    $items=$submits['items'];
    $rows=SubjectsWtypes::findById( $items );
...
function active(){
    ...
    if( $errmsg = SubjectsWtypes::setActive($id) ){
...
function inactive(){
    ...
    $errmsg = SubjectsWtypes::setInactive($id);
    if( $errmsg === true ){
